...
> 课：L4 · 规格：[specs/04-usable.md「4. 重试」](../specs/04-usable.md) · 预算：40 行 · 前置：T16
...
- 默认重试 3 次，指数退避 `baseMs * 2^n`（500 / 1000 / 2000），上限 8s，加 ±20% 抖动。
- **只重试可恢复错误**：网络错误、超时、HTTP 429、5xx。
- 429 若带 `Retry-After` 响应头，按它给的时长等待（覆盖退避计算）。两行代码的事，但它是「别跟服务器对着干」这条原则的具体化。
