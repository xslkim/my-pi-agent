...
> **run1 对齐后的实际范围（2026-08-17）**：run1 一次通过验收 10/10，唯一真实失败是 bash 工具
> 被后台守护进程挂死（run1.md 失败表 #1）。依 T21 纪律，本任务实际只做了这一项加固：
> bash 工具改为以进程 `exit` 为准返回（宽限后主动 destroy 流），并发现 MSYS 后台任务不在
> Windows 进程树内、外部 taskkill 杀不到，最终由 bash 侧 `kill $(jobs -p)` 清场（退出码保留）。
> 下面预判清单中的其余各项，run1 中没有出现对应证据，均未实施。
