...
- 跟随旁白推进（共 8 行旁白）：第 1 行（「模型跑偏了，总不能干等」）期间整条链蓝色呼吸脉动；第 2 行（「按下 Ctrl+C」）高亮节点①（accent 蓝描边放大，其余变暗）；第 3 行（「取消信号 AbortSignal」）高亮节点②并显示信号波纹向右扩散；第 4 行（「一路贯穿」）节点③④同时蓝色描边预告；第 5 行（「贯穿到 fetch」）高亮节点③，蓝色脉冲沿箭头流动；第 6 行（「贯穿到工具」）高亮节点④；第 7 行（「没贯穿就僵住」）左卡红框闪烁两次、链条整体染红；第 8 行（「会话还在原地」）红色褪去、节点⑤与右卡绿色高亮。行切换平滑过渡。
...
- 跟随旁白推进（共 7 行旁白）：第 1 行（「是有上限的」）错误卡轻微上浮提示（仍半透明）；第 2 行（「上下文预算」）高亮节点①，节点上方叠加「第 1 轮…第 N 轮」小标签依次堆叠；第 3 行（「估算文本量」）高亮节点②；第 4 行（「超阈值裁最老轮次」）高亮节点③与④，显示蓝色阈值刻度线；第 5 行（「越近的对话越值钱」）最左侧「最老轮次」标签变暗并被剪刀动画裁掉；第 6 行（「请求照常发出」）节点⑤亮起绿色、链条整体染绿；第 7 行（「上限是六十四 K」）绿色褪去，错误卡升至全不透明并红框闪烁两次，节点①处对话气泡无限堆高。行切换平滑过渡。
