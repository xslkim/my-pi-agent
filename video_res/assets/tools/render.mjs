// termcast · 渲染器：cast（JSONL 时间轴）-> 课程同款终端 HTML -> Chrome 截帧 -> ffmpeg mp4。
// 用法：node render.mjs <castFile> <outMp4> [speed]
...
const COALESCE_MS = 140; // 相邻事件合并为一帧

// ---- 极简 ANSI 状态机：只认 2m/0m/31m/32m，其余剥掉 ----
...
.dim{color:#8b949e}.red{color:#ff7b72}.grn{color:#3fb950}
.cur{display:inline-block;width:11px;height:${FONT - 6}px;background:#58a6ff;vertical-align:-2px;margin-left:2px}
...
function shoot(html, png) {
  const tmp = path.join(os.tmpdir(), `tc-${Date.now()}-${Math.random().toString(36).slice(2)}.html`);
  ...
  if (r.status !== 0) throw new Error(`chrome failed: ${r.stderr?.toString().slice(0, 200)}`);
}

/** cast 事件 -> 帧序列，结尾多停 1.2s；speed>1 加速回放。
 *  大块输出按「行」展开为连续帧（90ms/行），时间锚定真实事件时间取 max——
 *  既保留真实节奏，又保证画面逐行滚动而不是一次跳一大块。 */
function castToFrames(castFile, speed, titleOverride) {
  const lines0 = fs.readFileSync(castFile, "utf8").trim().split("\n").map((l) => JSON.parse(l));
  const meta = lines0[0];
  const events = lines0.slice(1).filter((e) => !e.d?.includes("[shell]")); // [shell] 自证行太吵，去掉
  let raw = "";
...
async function render(outMp4, { title, frames }) {
  const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "tcframes-"));
  const list = [];
  for (let i = 0; i < frames.length; i++) {
    const png = path.join(tmp, `f${String(i).padStart(4, "0")}.png`);
    shoot(frameHtml(title, frames[i].raw) + (i === frames.length - 1 ? "" : ""), png);
...
if (cmd === "--pi") {
  ...
  const lines = [];
  let t = 0;
  const push = (d, dur) => { lines.push({ t, raw: lines.map((x) => x.raw).join("\n") + (lines.length ? "\n" : "") + d }); t += dur; };
  // 重构：逐条生成
  let acc = "";
  const sp = +speed || 1;
  const add = (d, dur) => { acc = acc ? acc + "\n" + d : d; lines.push({ t, raw: acc }); t += dur / sp; };
  for (const m of msgs) {
  ...
  await render(out, { title: "run1 · 26 min → timelapse", frames: lines });
} else {
