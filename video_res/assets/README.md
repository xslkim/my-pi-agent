...
| 文件 | 用在 | 时长 | 内容与来源 |
|---|---|---|---|
| `lesson1-talk/assets/curl-sse.mp4` | L1-B02 | 15.3s | 真实 `curl -N`（300B/s 限速）拉取 SSE 字节流 |
| `lesson1-talk/assets/talk-demo.mp4` | L1-B10 | 16.0s | 真机单发流式输出（暗色思考/逐字/token 行） |
| `lesson1-talk/assets/pi-scroll.mp4` | L1-B11 | 3.4s | pi `openai-completions.ts`（1577 行）真实源码滚动 |
| `lesson2-tools/assets/calc-tools.mp4` | L2-B11 | 15.2s | 真机串行两次 calculator 调用（21×2 → 42+8） |
| `lesson2-tools/assets/pi-loop.mp4` | L2-B12 | 2.5s | pi `agent-loop.ts`（796 行）源码滚动 |
...
node render.mjs --pi   <源文件> <out.mp4> <起行> <止行>   # 源码滚动
